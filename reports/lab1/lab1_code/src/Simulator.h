...
private:
  ...
  struct IReg {
    ...
    bool predictedBranch;
    #ifdef SCOREBOARD_YYX
    bool ifClearScoreboard; // for control hazard
    bool ifTakenFUSuccessfully;
    bool ifBranchFlush;
    int FUNumber;
    #endif
  } iReg, iRegNew;
  // readOperand Stage below
  struct RReg {
    // Control Signals
    bool bubble;
    uint32_t stall;
  ...
  #ifdef SCOREBOARD_YYX
  // also reflected by this->iRegNew.ifTakenFUSuccessfully to indicate hazard
  bool ifStructureHazardBefore;
  bool ifWAWHazardBefore;
  #endif
  #endif
  ...
  const uint32_t latency[number_of_component_type] = {
    /* ALU */       0,
    /* memCalc */   1,
  ...
  struct History {
    ...
    uint32_t dataHazardCount; // yyx: actually just RAW hazard
    uint32_t controlHazardCount;
    uint32_t memoryHazardCount;

    #ifdef SCOREBOARD_YYX
    uint32_t structureHazardCount;
    uint32_t WAWHazardCount;
    uint32_t WARHazardCount;
    uint32_t controlFlushPipelineCount;
    #endif
  ...
  int regFU[RISCV::REGNUM];                  // default: -1, means none

  void clearFUScoreboardStatus(int FUNumber);
  int getFreeFUIntraNumber(int FUType); // -1, means none
  int calFUNumber(int FUType, int FUIntraNumber); // -1, means none
  void calFUTypeAndFreeFUIntraNumber(int FUNumber, int &FUType, int &FUIntraNumber);
  // yyx above
};
